...
def train(args):
    ...
    loss = model.fit(train_X, train_y, nb_epoch=args.epoch, batch_size=args.batch_size, callbacks=[checkpointer], shuffle="batch")
...
def val():
    ...
    for i, _ in enumerate(pred_classes):
        print (val_x[i])
        print("%d best top answer: %s with : %s " %(i,multi_val_y[i] ,  _[i]))
